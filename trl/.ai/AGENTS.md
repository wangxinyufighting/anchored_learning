...
The `[dev]` extra includes test, quality, and most optional dependencies (excluding vllm due to CUDA constraints).

## Common Commands

**Run tests:**
```bash
make test                    # Fast tests (parallel, excludes slow/low_priority)
make slow_tests              # Slow tests only
make test_experimental       # Experimental code tests
pytest tests/test_sft_trainer.py  # Single test file
pytest tests/test_sft_trainer.py::test_name  # Single test
```

**Lint and format:**
```bash
make precommit               # Run pre-commit hooks (adds copyrights, runs ruff)
```
...
Core trainers live in `trl/trainer/`:
- Each trainer has a paired config file (e.g., `sft_trainer.py` + `sft_config.py`)
- `base_trainer.py` provides minimal utilities (model card generation only)
- Trainers: `SFTTrainer`, `GRPOTrainer`, `DPOTrainer`, `RLOOTrainer`, `KTOTrainer`, `RewardTrainer`

Unstable/fast-evolving code lives in `trl/experimental/` and may change or be removed without notice.
